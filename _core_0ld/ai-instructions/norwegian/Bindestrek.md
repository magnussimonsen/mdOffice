...
> **Merknad om denne filen:** Dette er en opprydding av kildematerialet fra [Språkrådet – Bindestrek](https://sprakradet.no/godt-og-korrekt-sprak/rettskriving-og-grammatikk/tegn/bindestrek/) for bedre lesbarhet (for mennesker og AI). Endringer: overskrifter er markert med `#`/`##`/`###`, navigasjons- og skjemaelementer (inkludert "Relaterte sider") fra nettsiden er fjernet, og eksempler er satt i lister. Selve regelinnholdet er ikke endret.
...
Visse navn skal skrives **uten** bindestrek:
...
Ord som ligger i grenselandet mellom egennavn og fellesnavn (vanlige ord), kan skrives på begge måter:

- Maastrichttraktaten eller Maastricht-traktaten

En del vanlige fellesnavn der første del er et egennavn, skal skrives i ett ord:

- geigerteller, gudbrandsdalsost, norgesglass, røntgenapparat, selbuvott

### Forkortelser, bokstaver og siffer som førsteledd
...
Merk også: *NATO-land* eller *Nato-land* eller *natoland* (valgfritt).

I bøyde former av forkortelser o.l. og i avledninger bruker vi bindestrek slik:

- pc-en, pc-er, pc-ene (men: *en pc's prosessor*, se apostrof), AUF-er
...
Bindestrek settes også rett etter plusstegn og andre spesielle tegn:
...
| Situasjon | Bindestrek? | Eksempel |
|---|---|---|
| Utelatt del av ord (sammendragning) | Ja | bestefedre og -mødre |
| *ikke-* + ord (hovedregel) | Ja | ikke-røyker |
| *ikke* + ord (etablerte unntak) | Nei | ikkevold |
| Forhold/forbindelse mellom to ledd | Ja | romersk-katolsk |
| Forkortelse/bokstav som førsteledd | Ja | e-post, x-akse (unntak: ubåt) | 
| Siffer som førsteledd | Ja | 50-årsdag |
| Innlånt ord med etablert mønster | Nei | comebackforsøk |
| Geografisk sammensetning (type Nord-) | Ja | Nord-Europa |
| Egennavn som førsteledd i eget navn | Nei | Oslofjorden |
| Nylaget ord med egennavn | Ja | Clinton-vitser |
| Innarbeidet fellesnavn med egennavn først | Nei | gudbrandsdalsost |
