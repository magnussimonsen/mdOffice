...
- Check `pandoc` and `xelatex` on `PATH`
- Check required LaTeX packages listed in `_core/scripts/setup/latex-required-packages.txt`
- Create `.venv`
- Install `_core/scripts/requirements.txt`
- Create `.vscode/settings.json` with highlight rules
...
If you already have `.vscode/settings.json` and want to overwrite it:

```bat
_core\scripts\setup\install_windows.bat -Force
```
...
Both scripts define a Python command constant near the top (`$PythonCommand` / `PYTHON_CMD`).
If your machine uses a different Python alias/path, edit that value once and re-run.
...
| `vscode-highlight.json` | Optional: highlights `:::solution`/`:::blankbox` blocks and LaTeX math environments in the editor (needs the [Highlight](https://marketplace.visualstudio.com/items?itemName=fabiospampinato.vscode-highlight) extension) |
| `extensions.json` | Recommended extensions for this workspace |

Create a new file named "settings.json" in the `.vscode` folder if one doesn't already exist, then copy the template matching your OS into it.

**Step 4: Windows (PowerShell)**

```powershell
copy _core\vscode-settings-templates\settings.windows.json .vscode\settings.json
```

**Step 4: Linux / macOS**

```bash
cp _core/vscode-settings-templates/settings.linux.json .vscode/settings.json
```

To also enable the editor highlighting, merge `vscode-highlight.json`'s `highlight.regexes` key into your `.vscode/settings.json` alongside the keys copied above.
